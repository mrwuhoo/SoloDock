...
$installer = Join-Path (Get-Location) "dist.noindex/SoloDock-$version-windows-x64-setup.exe"
...
$installDir = Join-Path $testRoot 'SoloDock'
...
function Install-Panel {
  ...
  if (!(Test-Path "$installDir/SoloDock.exe")) { throw 'Installed executable missing' }
}
Install-Panel
node scripts/smoke-app.js "$installDir/SoloDock.exe" $profile
if ($LASTEXITCODE -ne 0) { throw 'Installed application smoke failed' }
Install-Panel
node scripts/smoke-app.js "$installDir/SoloDock.exe" $profile retained
...
if (Test-Path "$installDir/SoloDock.exe") { throw 'Uninstall left executable behind' }
