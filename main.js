...
const LEGACY_USER_DATA_PATH = path.join(app.getPath('appData'), 'TO-DO Panel Glass');
// Keep Electron's internal name stable: macOS safeStorage uses it for the
// existing Keychain entry. User-facing branding comes from the bundle and UI.
app.setName('TO-DO Panel Glass');
// Keep the native NSVisualEffectView on the same light material as the iOS
// glass palette. Otherwise a dark desktop/app can make the native layer dark
// while the renderer still uses dark navy text.
...
function createNotchTrayIcon() {
  if (process.platform === 'win32') return nativeImage.createFromPath(path.join(__dirname, 'build', 'solodock-icon.png')).resize({ width: 32, height: 32 });
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    frame: false,
    transparent: true,
    // Keep the native canvas fully transparent. macOS vibrancy materials add
    // an opaque system tint across the whole rectangular NSWindow, which both
    // hides the app behind it and leaks outside the CSS rounded corners.
    ...
    alwaysOnTop: true,
    skipTaskbar: true,
    // The native shadow follows the rectangular BrowserWindow surface rather
    // than the CSS panel mask and leaves dark square pixels below the rounded
    // corners. The CSS shell supplies the visible shadow instead.
    hasShadow: false,
    ...
    minimizable: false,
    maximizable: false,
    // A native rounded frameless window is automatically pushed below the Mac
    // menu bar when collapsed. CSS owns the visible bottom corner mask so the
    // 200px strip can remain exactly at the physical top edge.
    roundedCorners: false,
    show: false,
    webPreferences: {
      preload: path.join(__dirname, 'preload.js'),
