...
本文件记录 SoloDock 及其上游 TO-DO Panel 的重要功能、修复与发布变更。尚未发布的改动统一进入「未发布」，创建新版本标签时再归档到对应版本。
...
- 桌面品牌更名为 SoloDock，更新界面、权限提示、关于窗口及安装包显示名称。
- 新增冰蓝磨砂玻璃停靠图标、PNG 原图及 macOS ICNS 资源。
- 保留现有数据目录、Bundle ID、签名路径及钥匙串内部名称，避免纯改名引入数据迁移。
- README 明确衍生关系与未发布状态，保留上游 MIT 声明。
